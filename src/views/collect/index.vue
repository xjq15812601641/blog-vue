      ...
      <div v-if="articleList.length" v-loading="listLoading">
        ...
        >
          <div class="w-3/8 p-1 sm:p-4 flex relative justify-center align-center">
            <div :origin-url="item.imageUrl" alt="" class="w-full relative z-index-4 left-bg"></div>
            <div
...
<script setup lang="ts">
  ...
  import { useScrollTo } from "/@/modules/scroll-moduole/method/useScrollTo";
  ...
  const articleList = ref<Array<Article & { checked?: boolean; subject: string }>>([]);
  const go = useGo();
  ...
  // 监控元素实现图片懒加载
  const observe = () => {
    const imgList = document.querySelectorAll('.left-bg');
    ...
    var io = new IntersectionObserver((entries) => {
      entries.forEach((item) => {
        ...
        if (item.isIntersecting) {
          ...
          filename && useFileStream(filename, 'article-background', dom);
          // filename && getImgUrl(filename, dom);
          // 图片加载后即停止监听该元素
  ...
  const toDetail = (item) => {
    if (mode.value === 'operate') {
      ...
      go(`/${item.subject}/detail?id=${item.id}`);
    }
  };
</script>
