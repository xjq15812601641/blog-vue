...
export default (content?: Ref<string>, OwORef?: Ref<HTMLElement | null>) => {
  ...
  const handleEmoji = (content: string): string => {
    const str = content
      ...
      .replace(/\[((sgs)|(bili)|(bun))-[\u4e00-\u9fa5]+\]/g, (args: string): string => {
        ...
        return res
          ? `<img style="display: inline;margin: 0 2px;max-width:70px;max-height:70px" src="${
              isProdMode() ? res.url : 'https://shizuka.icu' + res.url
            }"/>`
          : '';
      });
    return str;
  ...
  /* 展示表情 */
  const showEmoji = (url: string) => {
    return isProdMode() ? url : `https://shizuka.icu${url}`;
  };
