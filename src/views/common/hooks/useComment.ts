...
import { createMessageComment, getMessageCommentList } from "/@/api/message/index";
...
export default (
  ...
  const content = ref('');
  const user = BlogCache.getCache(ProjectKeyEnum.userinfo_key) as GetUserInfoModel;
  ...
  const sendMsg = async (data: undefined | MessageComment) => {
    if (!BlogCache.getCache(ProjectKeyEnum.token_key)) return createMessage.error('只有登录后才能评论哦~');
    try {
      if (!data) {
        await createComment();
      } else {
  ...
  const handleLikeStatus = (item: MessageComment, type: string) => {
    if (!BlogCache.getCache(ProjectKeyEnum.token_key)) return false;
  ...
  const isShielded = (item: MessageComment) => {
    if (!BlogCache.getCache(ProjectKeyEnum.token_key)) return true;
